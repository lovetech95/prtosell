...
function Details() {
   ...
   const separator = (numb) => {
    ...
    const HandleData = (value,count) => {

        const data = { id: value._id,                                       
            name: value.name,
            photo: value.photos[0].dimensions.sm.url,  
            market: value.price.market,
            profit: 0.4*value.price.profit,
            quantity:count
        }

        //ModifyProduct(data,'ADD');

        window.open('https://docs.google.com/forms/d/e/1FAIpQLSfMiIUJHMlwPf0rETds58db9ai-L156uQp3BzeAYNzWFcQNww/viewform')
        //navigate(`/Giohang`)
                        ...
                        <div className='col-md-7 col-xs-12 info-product'>
                            ...
                            <div className='add-cart'>
                                <button type='button' className='add-items-cart btn btn-default'
                                        onClick={()=> HandleData(value,count)}
                                > 
